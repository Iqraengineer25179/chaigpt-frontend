...
import { aboutfeature } from '../../dummydata';
import { FaInfoCircle } from 'react-icons/fa';
import { Link } from 'react-router-dom';
import AboutImage from '../../AboutImage.png';
import FloatingParticle from '../flotingParticle/flotingParticle';
...
const AboutHome = () => {
  ...
  <p className='text-base sm:text-lg md:text-xl opacity-90 leading-relaxed max-w-3xl font-serif italic border-l-4 bg-amber-500/10 pl-4 sm:pl-6 py-2 bg-gradient-to-r from-white/5 to-transparent'>
...
<div className='grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-8 px-4 sm:px-0'>
  {aboutfeature.map((item, i) => (
    <div key={i} className='flex flex-col items-center justify-center gap-3 sm:gap-4 transition-transform duration-300 p-4 sm:p-5 hover:translate-x-2'>
      <div className={`rounded-full bg-gradient-to-br p-3 sm:p-4 ${item.color} transition-transform duration-300 group-hover:scale-110`}>
        <item.icon className='text-2xl text-white sm:text-3xl' />
      </div>
      <div className='text-center'>
        <h3 className='text-xl font-bold sm:text-2xl'>{item.title}</h3>
        <p className='text-sm opacity-80 sm:text-base'>{item.text}</p>
      </div>
    </div>
  ))}
</div>
...
<div className='w-full order-2 lg:order-1 md:max-w-md lg:max-w-none lg:w-7/12 mt-12 mb-10 lg:mb-0 relative group transform hover:scale-[1.01] transition-all duration-500'>
  <div className='relative rounded-[4rem] overflow-hidden border-4 border-amber-900/30 hover:border-amber-500/50 transition-colors duration-500 shadow-2xl shadow-black/50'>
    <div className='absolute inset-0 bg-gradient-to-br from-amber-400/15 via-transparent to-amber-600/10 pointer-events-none z-10'></div>
