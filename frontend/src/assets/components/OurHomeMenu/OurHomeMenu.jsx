...
const OurHomeMenu = () => {
    ...
    // Pull fresh backend items whenever the home page mounts
    useEffect(() => { fetchFoodList(); }, [fetchFoodList]);

    // Filter by selected category (case-insensitive), show up to 4 items
    const displayed = food_list
        ...
        <div className='bg-gradient-to-br from-[#1a120b] via-[#2a1e14] to-[#3e2b1d] min-h-screen py-16 px-4 sm:px-6 lg:px-8'>
            <div className='max-w-7xl mx-auto'>
                ...
                {loadingFood && food_list.length === 0 ? (
                    ...
                    <p className='text-center text-amber-300/60 font-cinzel text-lg py-8'>
                                            ...
                                            <div className='flex items-center gap-2'>
                                                {qty > 0 ? (
                                                    ...
                                                    <button onClick={() => addToCart(item, 1)}
                                                        className='bg-amber-900/40 px-4 py-1.5 rounded-full font-cinzel text-xs uppercase tracking-wider transition-transform duration-300 hover:scale-110 hover:shadow-lg border border-amber-800/50'>
                ...
                <div className='flex justify-center mt-16'>
                    <Link to='/menu'
                        className='bg-amber-900/30 border-2 border-amber-800/30 text-amber-100 px-8 sm:px-10 py-3 rounded-full font-cinzel uppercase tracking-widest transition-all duration-300 hover:bg-amber-800/40 hover:text-amber-50 hover:scale-105 hover:shadow-lg backdrop-blur-sm'>
                        Explore Full Menu
                    </Link>
                </div>
            </div>
