...
import { features, stats, teamMembers } from '../../dummydata'
...
<section className='py-12 px-4 md:px-8 relative'>
  ...
  <motion.div className='text-amber-500 mb-4 inline-block'
    ...
    <Icon className='w-12 h-12 text-amber-50' />
  </motion.div>
...
<div className='relative z-10 h-full flex flex-col items-center justify-center'>
  <motion.div className='mb-4 p-3 rounded-full bg-amber-900/30 border border-amber-600/30'
 ...
 <motion.div className='flex justify-center gap-4 pt-6' initial={{ scale: 0 }} whileInView={{ scale: 1 }}>
  {
    Object.entries(m.social).map(([p, url]) => (
      <a key={p} href={url} target="_blank" rel="noopener noreferrer" className="...">
        {({ twitter: <FaXTwitter className="w-6 h-6" />, instagram: <FaInstagram className="w-6 h-6" /> })[p]}
      </a>
    ))
  }
</motion.div>
